...
namespace DStarLite
{
    public class DStarLiteclass
    {
        ...
        public static void RunDStarLite(int sx, int sy, int gx, int gy, DStarLiteEnvironment env)
        {
            ...
            while (!sstart.Equals(sgoal))
            {
                ...
                env.MoveTo(new Coordinates(ssstate.x, ssstate.y), new Coordinates(sstart.x, sstart.y));
        ...
        // calculates the key计算关键
        /*
        Priority of a vertex = key   顶点优先级=key
        ...
        */
        ...
        // finds the succesor s' with the min (c(u,s')+g(s'))   用最小值(c(u,s')+g(s'))求后继s'
        // where cost from u to s' is 1 and returns the value  从u到s的代价是1并返回值
        ...
        class State
        {
            ...
            public LinkedList<State> GetSucc()
            {
                ...
                var a = S.GetLength(0);
                var b = S.GetLength(1);
                if (x + 1 < S.GetLength(0)) s.AddFirst(S[x + 1, y]);  //东
                if (y + 1 < S.GetLength(1)) s.AddFirst(S[x, y + 1]);  //北
                if (x - 1 >= 0) s.AddFirst(S[x - 1, y]);  //西
                if (y - 1 >= 0) s.AddFirst(S[x, y - 1]);  //南

                if (x + Math.Sqrt(2) < S.GetLength(0) && y + Math.Sqrt(2) < S.GetLength(1)) s.AddFirst(S[x + 1, y + 1]);  //东北
                if (x + Math.Sqrt(2) < S.GetLength(0) && y - Math.Sqrt(2) >= 0) s.AddFirst(S[x + 1, y - 1]);  //东南

                if (x - Math.Sqrt(2) > 0 && y + 1 < S.GetLength(1)) s.AddFirst(S[x - 1, y + 1]);  //西北
                if (x - Math.Sqrt(2) >= 0 && y - Math.Sqrt(2) >= 0) s.AddFirst(S[x - 1, y - 1]);  //西南
    ...
    {
        void MoveTo(Coordinates s, Coordinates e);
